...
ukb.fits <- fread(paste0('~/ukb/prsice_res/',pop,'_imputed.dat'))
ptr <- match( pheno, ukb.fits$V1 )
ukb.fits <- ukb.fits[ptr,]
...
if( pheno.bbj=="Height" ){
    data <- data.frame( height$height, bmi$Age, height$gender, pcs[,-1] )
}
if( pheno.bbj=="BMI" ){
    data <- data.frame( bmi$BMI, bmi$Age, height$gender, pcs[,-1] )
}
if( pheno.bbj=="LDL-C" ){
    data <- data.frame( ldl$LDL, ldl$Age, height$gender, pcs[,-1] )
}
if( pheno.bbj=="MCV" ){
    data <- data.frame( blood$mcv, ldl$Age, height$gender, pcs[,-1] )
}
if( pheno.bbj=="Plt" ){
    data <- data.frame( log(blood$plt), ldl$Age, height$gender, pcs[,-1] )
}
if( pheno.bbj=="Mono" ){
    data <- data.frame( log(blood$mono), ldl$Age, height$gender, pcs[,-1] )
}
if( pheno.bbj=="Neutro" ){
    data <- data.frame( log(blood$neutro), ldl$Age, height$gender, pcs[,-1] )
}
if( pheno.bbj=="Eosino" ){
    data <- data.frame( blood$eos, ldl$Age, height$gender, pcs[,-1] )
}
if( pheno==30070 ){
    data <- data.frame( log(blood$rdw), ldl$Age, height$gender, pcs[,-1] )
}
...
pop.ld <- ukb.fits$V5
pop.ld <- ifelse( pop.ld=='eas', 'EAS', pop.ld )
pop.ld <- ifelse( pop.ld=='eur', 'EUR', pop.ld )
tmp <- fread(paste0(my.biome.dir,'preds/by_chr/prsice/',pop,'.',pop.ld,'_',pheno,
                    '_prsice_chr1.sscore'))
pred <- data.frame( tmp$IID, tmp$SCORE1_AVG*tmp$ALLELE_CT )
for( chr in 2:22 ){
    tmp <- fread(paste0(my.biome.dir,'preds/by_chr/prsice/',pop,'.',pop,'_',pheno,
                        '_prsice_chr',chr,'.sscore'))
...
write.table(t(r2.ci), append=TRUE,
            paste0('/hpc/users/hoggac01/biome/prsice_res/',pop,'.dat'),
