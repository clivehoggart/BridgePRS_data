...
if( herit=="25" ){
    ...
    outdir="cs_res/"
    dir <- paste0(dir,"/scores/")
}
if( herit=="50" ){
    cs.dir <- paste0(dir,'/cs50/',pop,'.',pheno,'.',size,'.',causal)
    cs.dir.eur <- paste0(dir,'/cs50/eur.',pheno,'.80k.',causal)
    outdir="cs_res50/"
    dir <- paste0(dir,"/scores50/")
}
if( herit=="75" ){
    cs.dir <- paste0(dir,'/cs75/',pop,'.',pheno,'.',size,'.',causal)
    cs.dir.eur <- paste0(dir,'/cs75/eur.',pheno,'.80k.',causal)
    outdir="cs_res75/"
    dir <- paste0(dir,"/scores75/")
}
if( herit=="half_n" ){
    cs.dir <- paste0(dir,'/cs_half_n/',pop,'.',pheno,'.',size,'.',causal)
    cs.dir.eur <- paste0(dir,'/cs_half_n/eur.',pheno,'.80k.',causal)
    outdir="cs_res_half_n/"
    dir <- paste0(dir,"/scores_half_n/")
}
if( herit=="50_half_n" ){
    cs.dir <- paste0(dir,'/cs50_half_n/',pop,'.',pheno,'.',size,'.',causal)
    cs.dir.eur <- paste0(dir,'/cs50_half_n/eur.',pheno,'.80k.',causal)
    outdir="cs_res50_half_n/"
    dir <- paste0(dir,"/scores50_half_n/")
}
